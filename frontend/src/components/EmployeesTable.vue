<template>
  <div class="employees-table">
    <!-- Шапка с заголовком, счётчиком и кнопками экспорта -->
    <div class="table-header">
      ...
      <div class="header-actions">
        ...
        <button
          ...
          @click="sendToGoogleSheets"
          :disabled="isExporting"
          title="Отправить напрямую в Google Таблицы"
        >
          ...
          <table>
            ...
            <tbody>
              <template v-for="group in getGroupsByAlliance(alliance)" :key="group">
                ...
                <tr
                  v-for="employee in getEmployeesByGroup(alliance, group)"
                  :key="employee.originalIndex"
                  class="employee-row"
                >
                  ...
                  <td class="center-col">
                    <div class="percentage-bar">
                      <div
                        ...
                        :style="{ width: employee.metrics.weekdayPercentage + '%' }"
                      ></div>
                  ...
                  <td class="center-col">
                    <div class="percentage-bar">
                      <div
                        class="percentage-fill weekend"
                        :style="{ width: employee.metrics.weekendPercentage + '%' }"
                      ></div>
                  ...
                  <td class="actions-cell">
                    ...
                    <button
                      ...
                      @click="showDetails(employee.originalIndex)"
                      title="Редактировать"
                    >
                      <svg viewBox="0 0 24 24" fill="none">
                        <path
                        ...
                        />
                        <path
                          d="M18.5 2.5L21.5 5.5M9 15H12L20 7L17 4L9 12V15Z"
                          stroke="currentColor"
                          stroke-width="2"
                          stroke-linecap="round"
                        />
                    ...
                    <button
                      ...
                      @click="confirmDeleteEmployee(employee.originalIndex, employee.name)"
                      title="Удалить сотрудника"
                    >
                      <svg viewBox="0 0 24 24" fill="none">
                        <path
                          d="M19 7L18.1327 19.1425C18.0579 20.1891 17.187 21 16.1378 21H7.86224C6.81296 21 5.94208 20.1891 5.86732 19.1425L5 7M10 11V16M14 11V16M4 7H20M16 7L15 4H9L8 7"
                          stroke="currentColor"
                          stroke-width="2"
                          stroke-linecap="round"
                        />
    ...
    <!-- Модальное окно с результатом экспорта -->
    <div v-if="showResultModal" class="modal-overlay" @click="closeResultModal">
      <div class="modal-content" @click.stop>
        <div class="modal-header">
          <h3>{{ exportResult.success ? '✅ Экспорт выполнен' : '❌ Ошибка экспорта' }}</h3>
          <button class="close-btn" @click="closeResultModal">&times;</button>
        </div>
        <div class="modal-body">
          <p>{{ exportResult.message }}</p>
          <div v-if="exportResult.success" class="sheet-link">
            <p>Ваша таблица доступна по ссылке:</p>
            <!-- 🔥 ЗАМЕНИТЕ НА ВАШУ ССЫЛКУ -->
            <a :href="spreadsheetUrl" target="_blank" class="spreadsheet-link">
              {{ spreadsheetUrl }}
            </a>
            <div class="modal-actions">
              <button class="action-btn primary" @click="openSpreadsheet">
                📂 Открыть таблицу
              </button>
...
export default {
  ...
  data() {
    return {
      // URL вашего Google Apps Script (оставьте как есть или замените)
      googleScriptUrl:
        'https://script.google.com/macros/s/AKfycbx5w_1G0msz-T6XYbBLeePUaHTFBsLyYAgJLL2x1Lyh4thaDDu5mPfWkbY3MIsriF35Og/exec',

      // 🔥 ЗАМЕНИТЕ НА ВАШУ ССЫЛКУ НА GOOGLE ТАБЛИЦУ
      spreadsheetUrl: 'https://docs.google.com/spreadsheets/d/1y3N8UTLihi0OVvBZdxgoTZp_fbvoq1iSjf2_drBgMqE/edit',

      showResultModal: false,
      exportResult: {
        success: true,
        message: 'Данные успешно отправлены в Google Таблицы!'
      },
      isExporting: false
    }
  },
  ...
  methods: {
    ...
    getGroupCount(alliance, group) {
      return this.employeesList.filter(
        emp => emp.alliance === alliance && emp.group === group
      ).length
    },
    ...
    confirmDeleteEmployee(index, name) {
      scheduleStore.showModal(`Удалить сотрудника ${name} и все его смены?`, () => {
    ...
    sendToGoogleSheets() {
      ...
      fetch(this.googleScriptUrl, {
        ...
        .catch(error => {
          ...
          this.exportResult = {
            success: false,
            message: 'Ошибка при отправке данных. Проверьте консоль.'
          }
          this.showResultModal = true
        })
        .finally(() => {
